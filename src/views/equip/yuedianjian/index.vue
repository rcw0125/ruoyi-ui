<template>
  <div class="app-container">
    ...
    <el-radio :label="1">按周</el-radio>
     ...
     <el-menu-item index="2" >车间设备点检率</el-menu-item>
    ...
    <el-table v-loading="loading" :data="equipbiaozhunList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="设备名称" align="center"  v-if="tbvisible" prop="sbname" width="180">
      ...
      <el-table-column label="点检路线" align="center"  v-if="tbvisible" prop="name" />
      <el-table-column label="点检单位" align="center" prop="dept"  />
      <el-table-column label="点检班组" align="center" v-if="teamvisible" prop="team"  />
      <el-table-column label="点检类别" align="center" v-if="tbvisible" prop="leibie" />
      <el-table-column label="点检周期" align="center" v-if="tbvisible" prop="zhouqi"  />
      <el-table-column label="应点检次数" align="center" prop="zhoucishu"   />
      <el-table-column label="实际点检次数" align="center" prop="djcs"   />

      <el-table-column label="点检率" align="center" prop="djcs"   >
       <template slot-scope="scope">
          
            <span>{{ (scope.row.djcs*100/ scope.row.zhoucishu).toFixed(1)}}</span>
  ...
  methods: {
    ...
    getList() {
      if(this.queryParams.note==""||this.queryParams.note==null){
        this.$message.error('月份不能为空，请先选择月份！');
        return;
      }
      ...
      listYueMingXi(this.queryParams).then(response => {
    ...
    listDeptRate() {
      ...
      //this.queryParams.lasttime='2021-12';
       console.log(this.queryParams.note);
      listDeptRate(this.queryParams).then(response => {
        this.equipbiaozhunList = response.rows;
        this.total = response.total;
        this.loading = false;
          });
