  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:guzhangtongji:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:guzhangtongji:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:guzhangtongji:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['system:guzhangtongji:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="guzhangtongjiList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="登记数量" prop="djsl">
          <el-input v-model="form.djsl" placeholder="请输入登记数量" />
        </el-form-item>
        <el-form-item label="应接收数量" prop="yjssl">
          <el-input v-model="form.yjssl" placeholder="请输入应接收数量" />
        </el-form-item>
        <el-form-item label="接收数量" prop="jssl">
          <el-input v-model="form.jssl" placeholder="请输入接收数量" />
        </el-form-item>
        <el-form-item label="处理数量" prop="clsl">
          <el-input v-model="form.clsl" placeholder="请输入处理数量" />
        </el-form-item>
        <el-form-item label="待处理数量" prop="dclsl">
          <el-input v-model="form.dclsl" placeholder="请输入待处理数量" />
        </el-form-item>
        <el-form-item label="应验证数量" prop="yyzsl">
          <el-input v-model="form.yyzsl" placeholder="请输入应验证数量" />
        </el-form-item>
        <el-form-item label="验证数量" prop="yzsl">
          <el-input v-model="form.yzsl" placeholder="请输入验证数量" />
        </el-form-item>
        <el-form-item label="验证率" prop="yzrate">
          <el-input v-model="form.yzrate" placeholder="请输入验证率" />
        </el-form-item>
        <el-form-item label="延误数量" prop="ywsl">
          <el-input v-model="form.ywsl" placeholder="请输入延误数量" />
        </el-form-item>
        <el-form-item label="延误率" prop="ywrate">
          <el-input v-model="form.ywrate" placeholder="请输入延误率" />
        </el-form-item>
        <el-form-item label="接收率" prop="jsrate">
          <el-input v-model="form.jsrate" placeholder="请输入接收率" />
        </el-form-item>
        <el-form-item label="处理率" prop="clrate">
          <el-input v-model="form.clrate" placeholder="请输入处理率" />
        </el-form-item>
        <el-form-item label="周" prop="zhou">
...
<script>
import { listGuzhangtongji, getGuzhangtongji, delGuzhangtongji, addGuzhangtongji, updateGuzhangtongji, exportGuzhangtongji } from "@/api/system/guzhangtongji";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getGuzhangtongji(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateGuzhangtongji(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addGuzhangtongji(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除设备故障统计编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delGuzhangtongji(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有设备故障统计数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
