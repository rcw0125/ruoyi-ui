  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="项目分类" prop="xmfl">
        <el-select v-model="queryParams.xmfl" placeholder="请选择项目分类" clearable size="small">
          <el-option
      ...
      <el-form-item label="提出单位" prop="tcrdw">
        <el-select v-model="queryParams.tcrdw" placeholder="请选择提出单位" clearable size="small">
          <el-option
      ...
      <el-form-item label="属性" prop="shuxing">
        <el-select v-model="queryParams.shuxing" placeholder="请选择属性" clearable size="small">
          <el-option
            v-for="dict in shuxingOptions"
            :key="dict.dictValue"
            :label="dict.dictLabel"
            :value="dict.dictLabel"
          />
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:zizhugaishan:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:zizhugaishan:remove']"
        >删除</el-button>
      </el-col>
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:zizhugaishan:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="zizhugaishanList"  @row-dblclick="dbSelected" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="项目分类" align="center" prop="xmfl"  />
       <el-table-column label="属性" align="center" prop="shuxing" />
      <el-table-column label="录入日期" align="center" prop="tctime" width="130"/>      
      ...
      <el-table-column label="完成日期" align="center" prop="wctime" width="130"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="属性" prop="shuxing">
          <el-select v-model="form.shuxing" placeholder="请选择属性">
            <el-option
              v-for="dict in shuxingOptions"
              :key="dict.dictValue"
              :label="dict.dictLabel"
    ...
    <el-dialog :title="title" :visible.sync="kesopen" width="600px" append-to-body>
      <el-form ref="kesform" :model="kesform" :rules="rules" label-width="80px">
        ...
        <el-form-item label="车间不通过原因" prop="reason">
          <el-input v-model="kesform.reason"  type="textarea"  placeholder="请输入不通过原因" />
        </el-form-item>
        <el-form-item label="科室不通过原因" prop="ksreason">
          <el-input v-model="kesform.ksreason"  type="textarea"  placeholder="请输入不通过原因" />
        </el-form-item>
        <el-form-item label="审核科室"  prop="shks">
          <el-select v-model="kesform.shks" placeholder="选择审核科室">
                <el-option
                  v-for="item in shksOptions"
                  :key="item.value"
                  :label="item.label"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary"  v-hasPermi="['system:zizhugaishan:remove']" @click="submitkesForm">确 定</el-button>
        <el-button @click="kescancel">取 消</el-button>
      </div>
    </el-dialog>
...
<script>
import {getKesList,listZizhugaishan, getZizhugaishan,getInfo, delZizhugaishan,getUpdateInfo, addZizhugaishan, updateZizhugaishan, exportZizhugaishan,getChjInfo,getKesInfo,getJybInfo,chjShh,kesShh,jybShh } from "@/api/system/zizhugaishan";
import ImageUpload from '@/components/ImageUploadZzgs';
...
export default {
  ...
  created() {
    ...
    this.getDicts("jygsshuxing").then(response => {
      this.shuxingOptions = response.data;
    });
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listZizhugaishan(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
   ...
   /** 车间审核 */
    submitchejForm() {
      this.$refs["chejform"].validate(valid => {
        if (valid) {
          if (this.chejform.id != null) {
            if(this.chejform.status=="车间审核未通过"){
              if(this.chejform.reason==null){
                 this.$message.error('请填写未通过原因！');
                 return;
              }
            }
            else{
               if(this.chejform.shks==null){
                 this.$message.error('请选择需要审核的科室！');
                 return;
              }
            }
            chjShh(this.chejform).then(response => {
              this.msgSuccess("车间审核成功");
              this.chejopen = false;
              this.getList();
            });
          } 
   ...
   /** 科室审核 */
    submitkesForm() {
        ...
        if (valid) {  
         updateZizhugaishan(this.kesform).then(response => {
