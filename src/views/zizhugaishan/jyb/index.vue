<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="zizhugaishanList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="编号" align="center" prop="id" />
      <!-- <el-table-column label="项目编号" align="center" prop="xmcode" /> -->
      <el-table-column label="改善点" align="center" prop="xmgsd" />
      <el-table-column label="项目分类" align="center" prop="xmfl"  />
      <el-table-column label="提出日期" align="center" prop="tctime" width="180"/>      
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="提出日期"  prop="tctime">
...
<script>
import { getKesList,getJybList,listZizhugaishan, getZizhugaishan, delZizhugaishan,getUpdateInfo, addZizhugaishan, updateZizhugaishan, exportZizhugaishan,getChjInfo,getKesInfo,getJybInfo,chjShh,kesShh,jybShh } from "@/api/system/zizhugaishan";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      getJybList(this.queryParams).then(response => {
      ...
      this.$refs["kesform"].validate(valid => {
        if (valid) {
          if (this.kesform.id != null) {
            if(this.kesform.status=="精益办审核未通过"){
              if(this.kesform.reason==null){
                 this.$message.error('请填写未通过原因！');
                 return;
              }
            }
            jybShh(this.kesform).then(response => {
              this.msgSuccess("精益办审核成功");
              this.kesopen = false;
              this.getList();
            });
          } 
