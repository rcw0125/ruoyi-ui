<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item v-show="false" label="检查属性" prop="checkprop">
        <!-- <el-select
          @change="handleQuery"
      ...
      <el-form-item label="状态" prop="status">
        ...
        <el-select @change="handleQuery" v-model="queryParams.status" placeholder="状态">
                <el-option
    ...
    >
      ...
      <el-table-column label="整改结果" align="center" prop="modresult" width="155" />
...
export default {
  ...
  created() {
    this.$set(this.queryParams, "checkprop", "跑冒滴漏");
    this.getList();
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listBzhjc(this.addDateRange(this.queryParams, this.dateRange)).then((response) => {
    ...
    handleUpdate(row) {
      ...
      getBzhjc(id).then((response) => {
        ...
        if(response.data.checkaccount!=this.$store.getters.name){
        this.$message.error('你不是检查人，不能修改！');
